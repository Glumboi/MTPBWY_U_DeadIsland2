...
namespace MTPBWY_U_DeadIsland2;
...
{
    public override string PluginDebugIdentifier { get; set; } = "MTPBWY_U_JediFallenOrder";
    ...
    private const string _engineIniLocation = @"%USERPROFILE%\AppData\Local\DeadIsland\Saved\Config\WindowsNoEditor";
    ...
    {
        return Directory.Exists(Environment.ExpandEnvironmentVariables(@"%USERPROFILE%\DeadIsland\Saved\SaveGames"));
    }

    public void OpenGameSaveLocation()
    {
        Process.Start("explorer.exe", Environment.ExpandEnvironmentVariables(@"%USERPROFILE%\AppData\Local\DeadIsland\Saved\SaveGames"));
    }
